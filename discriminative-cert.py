...
def prompt_builder(question, answer, path, mode):
    if mode == "zero-shot":
        ...
        path_string = utils.path_to_string(path)
        query = ZERO_COT_PROMPT.format(question=question, answer=answer,  path=path_string)
    elif mode == "few-shot":
        path_string = utils.path_to_string(path)
        query = FEWSHOT_PROMPT.format(question=question, answer=answer, path=path_string)
    elif mode == "few-shot-cot":
        path_string = utils.path_to_string(path)
        query = FEWSHOT_COT_PROMPT.format(question=question, answer=answer,  path=path_string)
    ...
    for p in ground_truth_paths:
        ...
        if args.eval_neg:
            ...
            if "YES" in response.upper() and "NO" not in response.upper():
                prediction = 1
        result_list.append({'path': p, 'prediction': prediction, "raw_response": response, "raw_input": query})
    avg_result = float(sum([r['prediction'] for r in result_list]))/len(result_list)
    result = {'id': data_id, "ground_answer": answer, "question": question, 'acc': avg_result, "details": result_list}
    return result
...
def main(args, LLM):
    ...
    if args.wandb:
        wandb.init(config=args,project="discriminative-cert", name=f"{input_file_name}_{args.model_name}_{args.mode}{args.postfix}")
    
    if args.n > 1:
        with ThreadPool(args.n) as p:
            with tqdm(p.imap_unordered(partial(predict, args=args, processed_list=processed_list, model=model), df.iterrows()), total=len(df)) as phar:
                for r in phar:
                    if r is None:
                        continue
    ...
    with open(os.path.join(output_dir, f"results_{args.mode}{args.postfix}.txt"), 'w') as fout:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_name', '-m', type=str, help="model name", default='gpt-3.5-turbo')
    parser.add_argument("--mode", default="zero-shot", choices=["zero-shot", "zero-shot-cot", "few-shot", "few-shot-cot", "neg-few-shot", "neg-few-shot-cot", "neg-reorder-zero-shot","neg-reorder-few-shot", "neg-reorder-few-shot-cot"])
    parser.add_argument("--debug", action="store_true")
    parser.add_argument("--force", action="store_true")
    parser.add_argument("--wandb", default=False, type=lambda x:bool(distutils.util.strtobool(x)), help="enable wandb")
    parser.add_argument("--eval_neg", default=False, type=lambda x:bool(distutils.util.strtobool(x)), help="enable wandb")
