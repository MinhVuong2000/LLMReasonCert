...
def bfs_with_rule(graph, start_node, target_rule):
    result_paths = []
    queue = deque([(start_node, [])])  # Use queues to store nodes to be explored and corresponding paths
    while queue:
...
def query(message, model="gpt-3.5-turbo"):
    ...
    if prompt_len > token_limitation:
        raise ValueError(f"Message length {prompt_len} exceeds token limitation {token_limitation}. Message: {message}")
    while True:
        try:
