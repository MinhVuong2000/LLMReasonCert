...
for data_path in glob.glob(os.path.join(result_path, "*")):
    ...
    for model_path in glob.glob(os.path.join(data_path, "*")):
        ...
        result_dict = {"zero-shot": None, "zero-shot-cot": None, "few-shot": None, "few-shot-cot": None}
        for prediction_path in glob.glob(os.path.join(model_path, "*.jsonl")):
            ...
            with open(prediction_path, "r") as f:
                ...
                for key in result_dict.keys():
                    result_key = os.path.basename(prediction_path).split("_")[1].split(".")[0]
                    if key == result_key:
        ...
        #                         result_dict[key] = auc
        print(f"{model_name} {result_dict['zero-shot']} {result_dict['zero-shot-cot']} {result_dict['few-shot']} {result_dict['few-shot-cot']}")
